...
extension UIKeyCommand {
    ...
    // We need appDelegate here only to populate the #selector actions for each command. Ideally we would not reach over to appDelegate from here, but there is no way to reach these methods without doing so.
